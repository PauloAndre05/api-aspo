...
export class CriarAgendamentoUseCase {
    ...
    async execute({ dataAgenda, servicoId, postoId, horarioId, telefone, email }: ICriarAgendamentoDTO): Promise<Agendamento> {
        ...
        const time = await this.agendamentoRepository.findTimeById(horarioId)
        ...
        const totalBookingsForTheTime = await this.agendamentoRepository.getTotalBookingsForTheTime(dataAgenda, horarioId, postoId);

        if(totalBookingsForTheTime === 10) {
            throw new RequestError("Limite agendamente atingido neste horário", 400)
        }

        return await this.agendamentoRepository.create({ dataAgenda, servicoId, postoId, horarioId, telefone, email })        
    }
}